...
type TxtFormat struct {
	...
	CategoryRegex *regexp.Regexp // 分类
	TagsRegex     *regexp.Regexp // 关键字/标签
}
...
func NewTxtFormat() *TxtFormat {
	return &TxtFormat{
		...
		// 分类：分类、类型、Category等
		CategoryRegex: regexp.MustCompile(`(?i)^\s*(?:分类|类型|Category|类别)\s*[：:\s]+(.+)$`),

		// 关键字/标签：标签、关键字、Tags、关键词等
		TagsRegex: regexp.MustCompile(`(?i)^\s*(?:标签|关键字|关键词|Tags|关键词)\s*[：:\s]+(.+)$`),
	}
}
...
func (tf *TxtFormat) ExtractMetadata(line string) (key, value string) {
	...
	// 检查分类
	if matches := tf.CategoryRegex.FindStringSubmatch(line); matches != nil {
		return "category", strings.TrimSpace(matches[1])
	}

	// 检查关键字/标签
	if matches := tf.TagsRegex.FindStringSubmatch(line); matches != nil {
		return "tags", strings.TrimSpace(matches[1])
	}
